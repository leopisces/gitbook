...
# 搜索ingress-nginx版本
$ helm search repo ingress-nginx -l|head -4

# 拉取指定版本
$ helm pull ingress-nginx/ingress-nginx --version 4.2.5 --untar
```
> 注意：
```js
# 拉不下来用这个
...
## 2、修改目录下的values.yaml文件【LoadBalancer方式】
> 需要修改的地方
...
# 更新ingress-nginx
$ helm upgrade ingress-nginx -n -f values.yaml -n ingress-nginx .
```

## 4.测试网页 test-nginx.yaml
...
## 5、windows测试【修改系统Host文件】
域名修改在windows系统中 C:\Windows\System32\drivers\etc
