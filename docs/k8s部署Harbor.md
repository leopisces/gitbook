...
## 1、前提条件
- Heml安装工具
- [完成动态存储挂载](docs/k8s存储动态挂载.md)
- [部署好ingress-nginx](docs/ingress-nginx部署.md)

## 2、添加Harbor 官方Helm Chart仓库
```js
$ helm repo add harbor  https://helm.goharbor.io
# 列出最新版本的包
$ helm search repo harbor -l |  grep harbor/harbor  | head  -4
# 下载Chart包到本地
$ helm pull harbor/harbor --version 1.10.2 --untar
```
...
expose:
  ...
  tls:
    ...
    secret:
      ...
      secretName: "harbor.leopisces.cn"                 # tls Sercret
      # The name of secret which contains keys named:
      # "tls.crt" - the certificate
      # "tls.key" - the private key
      # Only needed when the "expose.type" is "ingress".
      notarySecretName: "harbor.leopisces.cn"           # tls Sercret
  ingress:
    hosts:
      core: harbor.leopisces.cn      # 你的域名
      notary: notary.leopisces.cn    # 你的域名
    ...
    kubeVersionOverride: ""
    className: "nginx"      # 这个值看你ingress-ningx的设置
    annotations:
...
externalURL: https://harbor.leopisces.cn:31002          # 你的域名+ingress-nginx代理的端口
...
persistence:
  ...
  persistentVolumeClaim:
    registry:
      ...
      storageClass: "nfs-storage"                     # 填写动态存储的名称
      subPath: ""
